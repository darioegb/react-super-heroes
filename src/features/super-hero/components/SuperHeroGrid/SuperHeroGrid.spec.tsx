...
import { SuperHeroContext, SuperHeroProvider } from 'features/super-hero/context';
...
describe('SuperHeroGrid', () => {
  ...
  it('should render SuperHeroGrid', () => {
    const { container } = render(<SuperHeroProviderHost children={<SuperHeroGrid />} />);
    expect(container).toMatchSnapshot();
  });

  it('should render SuperHeroGrid without data when not exist data', () => {
    const { container } = render(<SuperHeroProvider children={<SuperHeroGrid />} />);
    expect(container.querySelectorAll('tr').length).toBe(2);
  });

  it('should cahnge sort when click on any colunm', () => {
    render(<SuperHeroProviderHost children={<SuperHeroGrid />} />);
